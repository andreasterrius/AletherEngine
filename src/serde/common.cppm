...
void write_transform(rapidjson::Value &parent_json, const std::string &key,
  ...
  parent_json.AddMember(rapidjson::Value(key.c_str(), allocator), transform_obj,
                        allocator);
}
...
void write_ambient_light(rapidjson::Value &parent_json, const std::string &key,
                         const AmbientLight &light,
                         rapidjson::Document::AllocatorType &allocator) {

  rapidjson::Value obj(rapidjson::kObjectType);
  write_float(obj, "intensity", light.intensity, allocator);
  write_vec<vec3>(obj, "background_color", light.background_color, allocator);
  write_vec<vec3>(obj, "color", light.color, allocator);
