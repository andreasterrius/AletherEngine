...
ale::Gizmo::Gizmo() : basicColorShader(afs::root("src/gizmo/basic_color_shader.vs").c_str(),
	...
	this->models.emplace_back(afs::root("resources/gizmo/Plane_XY.glb")); //PlaneXY
	this->models.emplace_back(afs::root("resources/gizmo/Plane_XZ.glb")); //PlaneXZ
	this->models.emplace_back(afs::root("resources/gizmo/Plane_YZ.glb")); //PlaneYZ
...
//			vec3 unitVecB = this->initialClickInfo.lastFrameRayPlaneHitPos - this->position;
//
//			vec4 delta = QuaternionFromvec3Tovec3(unitVecB, unitVecA);
...
optional<Gizmo_GrabAxis> Gizmo::grabAxis(Ray ray) {
	...
	if (this->gizmoType == Translate || this->gizmoType == Scale) {
		...
		if (coll.has_value()) {
			return Gizmo_GrabAxis{
				.rayCollisionPosition = ray.resolveT(coll.value()),
				.activeAxis = X
			};
		}
		coll = ray.tryIntersect(transform, this->models[ArrowY].meshes[0].boundingBox);;
		if (coll.has_value()) {
			return Gizmo_GrabAxis{
				.rayCollisionPosition = ray.resolveT(coll.value()),
				.activeAxis = Y
		...
		coll = ray.tryIntersect(transform, this->models[ArrowZ].meshes[0].boundingBox);
		if (coll.has_value()) {
			return Gizmo_GrabAxis{
				.rayCollisionPosition = ray.resolveT(coll.value()),
				.activeAxis = Z
		...
		coll = ray.tryIntersect(transform, this->models[PlaneYZ].meshes[0].boundingBox);
		if (coll.has_value()) {
			return Gizmo_GrabAxis{
				.rayCollisionPosition = ray.resolveT(coll.value()),
				.activeAxis = YZ
		...
		coll = ray.tryIntersect(transform, this->models[PlaneXZ].meshes[0].boundingBox);;
		if (coll.has_value()) {
			return Gizmo_GrabAxis{
				.rayCollisionPosition = ray.resolveT(coll.value()),
				.activeAxis = XZ
		...
		coll = ray.tryIntersect(transform, this->models[PlaneXY].meshes[0].boundingBox);
		if (coll.has_value()) {
			return Gizmo_GrabAxis{
				.rayCollisionPosition = ray.resolveT(coll.value()),
				.activeAxis = XY
		...
		auto coll = ray.tryIntersect(transform, this->models[RotationYZ].meshes[0].boundingBox);
		if (coll.has_value()) {
			return Gizmo_GrabAxis{
				.rayCollisionPosition = ray.resolveT(coll.value()),
				.activeAxis = YZ
		...
		coll = ray.tryIntersect(transform, this->models[RotationXZ].meshes[0].boundingBox);;
		if (coll.has_value()) {
			return Gizmo_GrabAxis{
				.rayCollisionPosition = ray.resolveT(coll.value()),
				.activeAxis = XZ
		...
		coll = ray.tryIntersect(transform, this->models[RotationXY].meshes[0].boundingBox);;
		if (coll.has_value()) {
			return Gizmo_GrabAxis{
				.rayCollisionPosition = ray.resolveT(coll.value()),
				.activeAxis = XY
...
optional<vec3> Gizmo::rayPlaneIntersection(Ray ray, Gizmo_ActiveAxis activeAxis, vec3 planeCoord) {
...
vec3 Gizmo::handleTranslate(Gizmo_ActiveAxis activeAxis, vec3 rayPlaneHitPoint) {
