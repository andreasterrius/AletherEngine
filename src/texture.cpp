...
Texture::Texture(int width, int height) : width(width), height(height) {
    vector<vec4> emptyPixel(width * height, vec4(0.0));

    glGenTextures(1, &this->id);
    glBindTexture(GL_TEXTURE_2D, this->id);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_FLOAT, emptyPixel.data());
...
void Texture::replaceData(vector<vec4> &flatColorData) {
    glBindTexture(GL_TEXTURE_2D, this->id);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGBA, GL_FLOAT, flatColorData.data());
    glBindTexture(GL_TEXTURE_2D, 0);
}
