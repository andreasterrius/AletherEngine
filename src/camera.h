...
public:
    ...
    Camera(Camera_InputType inputType, glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f),
    ...
    Camera(float posX, float posY, float posZ, float upX, float upY, float upZ, float yaw, float pitch) : Front(
            glm::vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(SPEED), MouseSensitivity(SENSITIVITY), Zoom(ZOOM), inputType(FPS) {
...
private:
    ...
    void updateCameraVectors() {
        if(this->inputType == FPS) {
            ...
            this->Front = glm::normalize(this->arcballInput.ArcballTarget - this->Position);
            this->Right = glm::normalize(glm::cross(Front,
                                                    WorldUp));  // normalize the vectors, because their length gets closer to 0 the more you look up or down which results in slower movement.
